...
# wine_data[,wine_cols] <- lapply(wine_data[,wine_cols], factor, level = c(0, 1), labels = c("0", "1"))
...
hist_score_plot <- ggplot(wine_data, aes(x = Score)) + 
...
```{r}
lm_1 <- lm(Score ~ Price, data = wine_data)

summary(lm_1)
```

```{r}
lm_2 <- lm(Score ~ Year, data = wine_data)
