...
sample = sample.split(wine_fixed$Score, SplitRatio = .75)

train = subset(wine_fixed, sample == TRUE)
test = subset(wine_fixed, sample == FALSE)
...
bag_mod <- randomForest(Score ~., # Set tree formula
                ...
                mtry = 617, # Set mtry to number of variables 
                ntree = 200,
                na.action=na.exclude) # Set number of trees to use
bag_mod # View model
```
...
```{r}
system.time(fit <- train(Score~., method="rf",data=wine_fixed,trControl = fitControl,na.action=na.exclude))
```
